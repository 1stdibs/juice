...
function inlineDocument(document, css, options) {
  ...
  if (options.preserveMediaQueries) {
    ...
    mediaQueries = utils.getMediaQueryText(css);
    try {
      mQStyle.innerHTML = mediaQueries;
    } catch(e) {
      mQStyle.styleSheet.cssText = mediaQueries;
    }
    document.getElementsByTagName('head')[0].appendChild(mQStyle);
  }
